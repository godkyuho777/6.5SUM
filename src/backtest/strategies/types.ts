...
export const STRATEGY_REGISTRY = new Map<StrategyName, BacktestStrategy>();

/** 전략 등록 헬퍼 */
export function registerStrategy(s: BacktestStrategy): void {
  STRATEGY_REGISTRY.set(s.name, s);
}

/** 전략 lookup */
export function getStrategy(name: StrategyName): BacktestStrategy {
  const s = STRATEGY_REGISTRY.get(name);
  if (!s) {
    throw new Error(
      `[Backtest] Unknown strategy '${name}'. Available: ${Array.from(STRATEGY_REGISTRY.keys()).join(", ")}`,
    );
  }
  return s;
}

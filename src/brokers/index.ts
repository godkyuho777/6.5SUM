...
export function getDefaultBroker(): Broker {
  ...
  const env = (process.env.BROKER ?? "bybit").toLowerCase().trim();
  switch (env) {
    case "bybit":
    case "bybit-v5":
      cachedBroker = new BybitV5Broker();
      break;
    default:
      // 미지원 broker 명시 시에도 강제 throw X — 운영 차단 회피.
      // 운영자가 startup-validation 으로 인지 가능 (P2-#12 카테고리 추가 권장).
      console.warn(
        `[brokers] Unknown BROKER="${env}", falling back to bybit-v5`,
      );
      cachedBroker = new BybitV5Broker();
      break;
  }
 ...
 */
export function setDefaultBrokerForTests(broker: Broker | null): void {
  cachedBroker = broker;
}

...
export interface ShortPathThresholds {
  ...
  ptn: { bbTolerance: number; adxMax: number };
}
...
export const SIGNAL_THRESHOLDS: SignalThresholds = {
  long: {
    num: {
      rsiLow: 25, // BBDX NUM path: RSI < 25 + BB lower + ADX < 20 → strong oversold
      rsiHigh: 38, // RSI 25~38 = NUM-OK (pullback zone)
      bbTolerance: 0.02, // ±2% from BB lower
      adxMax: 20, // ADX < 20 = 약세장 (평균회귀 favorable)
    },
    ptn: {
      bbTolerance: 0.05, // ±5% from BB lower (NUM 보다 관대)
      adxMax: 25, // ADX < 25 (PTN 은 더 강한 추세도 허용)
    },
  },
  short: {
    num: {
      rsiLow: 65, // SHORT 대칭: RSI > 65 + BB upper + ADX < 20
      rsiHigh: 75, // RSI 65~75 = SHORT-NUM-OK
      bbTolerance: 0.02,
      adxMax: 20,
    },
    ptn: {
  ...
  exit: {
    rsiThreshold: 65, // RSI ≥ 65 → exit signal 가속
    adxThreshold: 30, // ADX ≥ 30 → 강한 추세 → exit 약화 (false negative ↓)
    plusDiThreshold: 25, // +DI ≥ 25 → 매수세 강 → exit 약화
  },
  vwap: {
    atTolerance: 0.001,
...
export function overrideThresholds(
  ...
  overrides: DeepPartial<SignalThresholds>,
): SignalThresholds {
  return {
    long: {
      num: { ...base.long.num, ...overrides.long?.num },
      ptn: { ...base.long.ptn, ...overrides.long?.ptn },
    },
    short: {
      num: { ...base.short.num, ...overrides.short?.num },
      ptn: { ...base.short.ptn, ...overrides.short?.ptn },
    },
    exit: { ...base.exit, ...overrides.exit },
    vwap: { ...base.vwap, ...overrides.vwap },
  };
}

...
import { liteRouter } from "./lite/router";
...
현재 시장 상황, 진입 적절성, 리스크 요인, 예상 시나리오를 간결하게 설명해주세요.`;
  ...
  //
  // PostgreSQL schema-level 에러 (42P01 relation does not exist, 42703 column
  // does not exist) → DB_UNAVAILABLE 로 변환하여 frontend mock fallback 가능하게.
  // Migration 0008 미적용 상태에서도 UI 가 graceful 유지 (헌장 R8 graceful 흐름).
    ...
    optIn: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          ...
          return categorizeLeaderboardError(err, "optIn");
        }
      }),
    ...
    optOut: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          ...
          return categorizeLeaderboardError(err, "optOut");
        }
      }),
    ...
    sync: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          ...
          return categorizeLeaderboardError(err, "sync");
        }
      }),
    ...
    fetch: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        try {
          ...
          return categorizeLeaderboardError(err, "fetch");
        }
      }),
  }),
...
// ─── Simulator Leaderboard Error Helper ─────────────────────────
/**
 * PostgreSQL schema-level 에러 (42P01 relation does not exist, 42703 column
 * does not exist) 를 DB_UNAVAILABLE 로 변환.
 *
 * 효과: Migration 0008 미적용 환경에서 frontend 의 graceful mock fallback 작동.
 * 사용자가 INTERNAL 에러 대신 friendly "DB 미적용" 메시지 + mock 표시.
 *
 * 그 외 에러는 INTERNAL 로 유지 — 로그 + 디버깅용.
 */
function categorizeLeaderboardError(
  err: unknown,
  operation: string,
):
  ...
  // PostgreSQL "errors" lib 가 code 를 직접 노출하거나, 메시지에 포함시킴
  ...
  if (isSchemaError) {
    console.warn(
      `[simulatorLeaderboard.${operation}] schema mismatch — DB_UNAVAILABLE fallback:`,
      msg,
    );
    return {
      ok: false as const,
      ...
      message:
        "Migration 0008 미적용 — Supabase Dashboard SQL Editor 에서 drizzle/migrations/0008_simulator_leaderboard.sql 실행 필요. (frontend 가 mock 으로 graceful fallback 됩니다.)",
    };
  }

  console.error(`[simulatorLeaderboard.${operation}] unexpected error:`, err);
  return {
    ok: false as const,
    code: "INTERNAL" as const,

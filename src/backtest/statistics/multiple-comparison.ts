...
export function benjaminiHochberg(
  ...
  // (p, original_index) 페어로 정렬
  const indexed = pValues.map((p, i) => ({ p, i }));
  indexed.sort((a, b) => a.p - b.p);

  // 가장 큰 k 찾기: p_(k) ≤ (k/m) × q
  let kStar = -1;
  for (let rank = 1; rank <= m; rank++) {
    const critical = (rank / m) * q;
    if (indexed[rank - 1].p <= critical) {
      kStar = rank;
    }
  }

  // ≤ kStar 인 모든 가설 reject
  const significant = new Array<boolean>(m).fill(false);
  if (kStar > 0) {
    for (let rank = 1; rank <= kStar; rank++) {
...
// ─────────────────────────────────────────────────────────
// User hypothesis registry (P2 후속: DB 통합 예정)
// ─────────────────────────────────────────────────────────
 ...
 */
export function applyMultipleComparisonCorrection<
  T extends { p_value: number; alpha_significant: boolean },
>(
  result: T,
  user_hypotheses: UserHypothesis[],
  q: number = 0.1,
): MultipleComparisonReport<T> {
  const others = user_hypotheses
    .filter((h) => h.status === "tested" && typeof h.p_value === "number")
    .map((h) => h.p_value!);
  const all = [...others, result.p_value];
  const sig = benjaminiHochberg(all, q);
  // 본 결과는 배열의 마지막
  const corrected = sig[sig.length - 1] ?? false;
  return {
    result,
    alpha_significant_after_correction: corrected,
    q,
    total_hypotheses: all.length,
    hypothesis_rank: all.length, // 시간순 등록 가정 — 정확한 순위는 DB 통합 후
  };
}
